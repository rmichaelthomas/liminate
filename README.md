...
Liminate is a programming language whose syntax is plain English. A small, bounded vocabulary of 54 reserved words combines into sentences that a real interpreter lexes, parses, type-checks, and runs. Not a prompt. Not a code generator. The prose IS the program.
...
The current build is **v0.7.0**: 19 verbs, 20 connectives, 8 operators, 1 declaration, 54 base reserved words, **1160 tests passing**.
...
A pack is a small JSON file that adds nouns and verbs while it's loaded. The base 54 words are permanent; pack-contributed words are reserved only when the pack is active.
...
- **The vocabulary is the boundary.** 54 base reserved words. Expressiveness scales through composition and domain packs, not through adding keywords.
