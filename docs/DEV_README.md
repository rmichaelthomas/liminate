...
2. **Bounded vocabulary as design constraint.** 51 reserved words in the current build (19 verbs + 19 connectives + 7 single-word operators + `equal`/`multiplied`/`divided` as multi-word components + 3 articles + 0 v2-deferred words — `V2_RESERVED` is now empty). The vocabulary is the language boundary, not a starter set that grows. Expressiveness scales through domain packs, composition over expansion, and named-composition chunking — not through adding more keywords. 
...
The current base vocabulary is 51 reserved words across five categories. The complete list is the entire language surface — no other words are part of Liminate, only user-provided names and literal values. Domain packs may add their own verbs and nouns at runtime via the pack verb contract; pack-contributed words are reserved only while the pack is loaded, and the base 51 are permanent.
