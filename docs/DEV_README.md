...
Liminate is a general-purpose programming language whose source code is readable English prose. A bounded vocabulary of 58 words combines into sentences that execute directly. There is no separate code the prose generates — the sentence IS the program.
...
1456 tests pass via `pytest tests/`. Each spec section that locks a behavior has at least one test that exercises it.
...
All 127 sentences are wired through the test suite. v1/v2a/v2b/v2c/v2d coverage lives in `tests/test_integration.py`; v3a in `tests/test_integration_v3a.py`; v4a in `tests/test_integration_v4a.py`. The full suite is 1456 tests.
