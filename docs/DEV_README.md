...
Liminate is a general-purpose programming language whose source code is readable English prose. A bounded vocabulary of 61 words combines into sentences that execute directly. There is no separate code the prose generates — the sentence IS the program.
...
The current base vocabulary is 61 reserved words across six categories. The complete list is the entire language surface — no other words are part of Liminate, only user-provided names and literal values. Domain packs may add their own verbs and nouns at runtime via the pack verb contract; pack-contributed words are reserved only while the pack is loaded, and the base 61 are permanent.
...
| `sum` | Sum the numbers in a list (non-destructive, auto-shows; empty list sums to 0). Renamed from `combine` in v0.15.0 — `combine` is tombstoned. |
...
Inside a `where` clause `each` is a pronoun for the current item, not the iteration verb. `sum` is non-destructive, so the final `remember ... from sum the numbers` captures 40 while `numbers` still contains `[6, 7, 8, 9, 10]`. The `from <verb-phrase>` construct triggers recursive descent in the parser.
...
│   ├── run.py                       Public `run()` entry point; Session; ContractResult (shared by CLI and Receipts)
│   ├── cli.py                       Display layer + REPL + `--pack` flag (only module with input/print)
...
1654 tests pass via `pytest tests/`. Each spec section that locks a behavior has at least one test that exercises it.
...
All 127 sentences are wired through the test suite. v1/v2a/v2b/v2c/v2d coverage lives in `tests/test_integration.py`; v3a in `tests/test_integration_v3a.py`; v4a in `tests/test_integration_v4a.py`. The full suite is 1654 tests (127 locked sentences plus coverage for every era and vocabulary wave since).
