...
2. **Bounded vocabulary as design constraint.** 54 reserved words in the current build (19 verbs + 20 connectives + 8 single-word operators + `equal`/`multiplied`/`divided` as multi-word components + 3 articles + 1 declaration + 0 v2-deferred words — `V2_RESERVED` is now empty). The vocabulary is the language boundary, not a starter set that grows. Expressiveness scales through domain packs, composition over expansion, and named-composition chunking — not through adding more keywords. 
...
| [`docs/language/syntax.md`](docs/language/syntax.md) | Full syntax tour: source-file rules, all nineteen verbs, lists, records, conditions, `each`, named compositions with parameters, `choose`, quoting, arithmetic, `sort`/`compare`/`transform`, and the v3a `when`/`unless`/`finish` listener model. |
