...
Liminate is a programming language whose syntax is plain English. A small, bounded vocabulary of 53 reserved words combines into sentences that a real interpreter lexes, parses, type-checks, and runs. Not a prompt. Not a code generator. The prose IS the program.
...
The current build is **v0.6.0**: 19 verbs, 20 connectives, 1 declaration, 53 base reserved words, **1119 tests passing**.
...
**Connectives (20):** `where`, `and`, `or`, `from`, `with`, `called`, `to`, `how`, `as`, `of`, `if`, `otherwise`, `when`, `unless`, `includes`, `within`, `over`, `then`, `by`, `because`. The last, `because`, attaches a quoted rationale to any verb statement as inert metadata — visible in canonical rendering, `inspect`, and Receipts, but never executed. Statement-terminal and per-statement: `require amount is above 50000 because "SOX compliance"`.

**Operators (7):** `is`, `above`, `below`, `not`, `plus`, `minus`, `reverse`. Multi-word: `equal to`, `multiplied by`, `divided by`.
...
A pack is a small JSON file that adds nouns and verbs while it's loaded. The base 53 words are permanent; pack-contributed words are reserved only when the pack is active.
...
- **The vocabulary is the boundary.** 53 base reserved words. Expressiveness scales through composition and domain packs, not through adding keywords.
